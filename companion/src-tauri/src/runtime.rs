...
use crate::target::{
    ForegroundProbe, ForegroundRestoreOutcome, ForegroundTargetRestorer, Target, TargetDecision,
    evaluate_target,
};
    ...
    pub fn poll_dry_run(&mut self) -> Result<RuntimeStatus, RuntimeError> {
        ...
        let event = match self.next_event() {
            ...
            Err(RuntimeError::Serial) => {
                return Ok(self.stop_with_last_event(RuntimeError::Serial.to_string()));
            }
...
mod tests {
    ...
    use crate::profile::{MappingDraft, ProfileSerial, ProfileTarget};
    use crate::target::{
